...
def gameSettings():
    ...
    while True:
        ...
        DISPLAY.blit(backgroundImg, [0, 0])
        displayMsg("Settings", GREEN, -230, "large")
        displayMsg(
            "Please do not modify settings with text editor!", YELLOW, -140)

        # display sound settings
        displayMsg("Game-time music", YELLOW, -110)
        if cdata[4]:
            displayMsg("On [press B]", YELLOW, -80)
        else:
            displayMsg("Off [Press B]", YELLOW, -80)
        displayMsg("Sound effects", YELLOW, -50)
        if cdata[5]:
            displayMsg("On [Press V]", YELLOW, -20)
        else:
            displayMsg("Off [Press V]", YELLOW, -20)
        ...
        if cursor_y <= HEIGHT/2:
            ...
            setKeyNo = 1
        displayMsg(
            "Move the cursor to upper or lower side to change key.", YELLOW, 10)
        displayMsg("You can not use key that are pre-defined!", YELLOW, 40)
        displayMsg("Set Key for: " + setKeyName, YELLOW, 70)
        displayMsg(
            "Current Key: " + pygame.key.name(cdata[setKeyNo]), YELLOW, 100)
